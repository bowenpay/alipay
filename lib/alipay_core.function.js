 ...
 * return 远程输出的数据
 */
exports.getWapHttpResponseGET = function(url,cacert_url, callback){
    var parsed_url = require('url').parse(url);
 ...
 * return 拼接完成以后的字符串
 */
exports.createWapLinkstring = function(para){
    //手机网站支付MD5签名固定参数排序，顺序参照文档说明
	var ls = '';
    ls += 'service=' + para['service'];
    ls += '&v=' + para['v'];
    ls += '&sec_id=' + para['sec_id'];
    ls += '&notify_data=' + para['notify_data'];
	return ls;
}
