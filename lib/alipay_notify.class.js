 ...
 * @return 签名验证结果
 */
AlipayNotify.prototype.getWapSignVeryfy = function(para_temp, sign){
    //除去待签名参数数组中的空值和签名参数
    var para_filter = core_funcs.paraFilter(para_temp);
    ...
    //把数组所有元素，按照“参数=参数值”的模式用“&”字符拼接成字符串
    var prestr = core_funcs.createWapLinkstring(para_sort);

    var isSgin = false;

    var sign_type = this.alipay_config['sec_id'].trim().toUpperCase();
    if(sign_type == "0001"){
        isSgin = md5_f.md5Verify(prestr, sign, this.alipay_config['key']);
    }
    else{
...
/**
 * 针对notify_url验证消息是否是WAP支付宝发出的合法消息
 * @return 验证结果
 */
AlipayNotify.prototype.verifyWapNotify = function(_POST, callback){
    if(Object.keys(_POST).length == 0) {//判断POST来的数组是否为空
        callback(false);
    }
    else{
        //生成签名结果
        var isSign = this.getWapSignVeryfy(_POST, _POST["sign"]);
        //获取支付宝远程服务器ATN结果（验证是否是支付宝发来的消息）
        var responseTxt = 'true';
        //验证
        //responsetTxt的结果不是true，与服务器设置问题、合作身份者ID、notify_id一分钟失效有关
