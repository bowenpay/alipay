...
function AlipaySubmit(alipay_config){
    ...
    this.alipay_wap_pay_gateway = 'http://wappaygw.alipay.com/service/rest.htm?';
}
...
AlipaySubmit.prototype.buildWapRequestParaToString = function(para_temp){
    //待请求参数数组
    var para = this.buildWapRequestPara(para_temp);

    //把参数组中所有元素，按照“参数=参数值”的模式用“&”字符拼接成字符串
    var request_data = core_funcs.createLinkstring(para);

    return request_data;
}
AlipaySubmit.prototype.buildWapRequestPara = function(para_temp){
    //除去待签名参数数组中的空值和签名参数
    var para_filter = core_funcs.paraFilter(para_temp);
    ...
    //签名结果与签名方式加入请求提交参数组中
    para_sort['sign'] = mysign;
    para_sort['sec_id'] = this.alipay_config['sec_id'].trim().toUpperCase();
...
AlipaySubmit.prototype.buildWapRequestHttp = function (para_temp, callback) {
    //待请求参数数组字符串
    var request_para = this.buildWapRequestParaToString(para_temp);
    console.log(this.alipay_wap_pay_gateway+request_para);
    //远程获取数据
    core_funcs.getWapHttpResponseGET(this.alipay_wap_pay_gateway+request_para, '', callback);    
}

AlipaySubmit.prototype.buildWapRequestForm = function (para_temp, method, button_name) {
    //待请求参数数组
    var para = this.buildWapRequestPara(para_temp);

    var sHtml = "<form id='alipaysubmit' name='alipaysubmit' action='"
            + this.alipay_wap_pay_gateway
            +  "_input_charset="
            + this.alipay_config['input_charset'].toLowerCase().trim()
            + "' method='" + method + "'>";
    ...
    //submit按钮控件请不要含有name属性
    sHtml = sHtml+ "<input type='submit' value='" + button_name + "'></form>";
    if(this.alipay_config['weixin_blocked']){
        sHtml = sHtml + '<script type="text/javascript" src="ap.js"></script>';
        sHtml = sHtml + '<script>var queryParam="";Array.prototype.slice.call(document.querySelectorAll("input[type=hidden]")).forEach(function(e){queryParam+="req_data"===e.name?e.name+"="+e.value+"&":e.name+"="+encodeURIComponent(e.value)+"&"});var gotoUrl=document.querySelector("#alipaysubmit").getAttribute("action")+"&"+queryParam;_AP.pay(gotoUrl);</script>';
    } else {
        sHtml = sHtml + "<script>document.forms['alipaysubmit'].submit();</script>";
    }
...
AlipaySubmit.prototype.getRequestToken = function (respHtml) {
    var res_data = qs.parse(qs.unescape(respHtml)).res_data;
    console.log(res_data);
    var doc = new DOMParser().parseFromString(res_data);
    var request_token = doc.getElementsByTagName("request_token").item(0).firstChild.nodeValue;
